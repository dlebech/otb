...
function SelectionIntro({
  ...
  return (
    <div className="d-inline-flex">
      ...
      <small className="text-muted cursor-pointer ms-2" onClick={handleSelectAll}><u>Select all rows</u></small>
      {selectedTransactions.length === 0 && 
        <small className="text-muted ms-2">(or click on a row&#39;s date or description to select it)</small>}
      {selectedTransactions.length >= 1 &&
        <>
          <small className="text-muted cursor-pointer ms-2" onClick={handleSelectNone}><u>Clear selection</u></small>
